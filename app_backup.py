...
from database import (
    ...
    add_deadline, get_all_deadlines, delete_deadline,
    save_reflection, get_reflection, get_all_reflections,
)
from calendar_sync import fetch_today_events

st.set_page_config(page_title="Goal Progress Tracker", page_icon="🔋", layout="wide")

# ----------------------------------------------------------------
# STYLING - headline font + status badge pills
# Palette: warm neutral background, deep teal for structure, amber for
# progress/energy (ties to the e-mobility/off-grid focus of the projects
...
st.markdown(
    ...
    .badge-done { background-color: #D9EEE9; color: #0F6466; }
    .badge-in_progress { background-color: #FBEBD0; color: #A6690F; }
    .badge-skipped { background-color: #EFEBE4; color: #6B6459; }
    .badge-applied { background-color: #E6ECF5; color: #2D4B72; }
    .badge-interview { background-color: #FBEBD0; color: #A6690F; }
    .badge-offer { background-color: #D9EEE9; color: #0F6466; }
    .badge-rejected { background-color: #F5E1DE; color: #A13E2E; }
    .badge-no_response { background-color: #EFEBE4; color: #6B6459; }
    </style>
    """,
...
def status_badge(status):
    """Return an HTML pill badge for a status value, colored by meaning."""
...
# ----------------------------------------------------------------
# COUNTDOWN BANNER - shown at the top of every page
# ----------------------------------------------------------------
deadlines = get_all_deadlines()
if deadlines:
    today = date.today()
    cols = st.columns(len(deadlines))
    for col, d in zip(cols, deadlines):
        deadline_date = date.fromisoformat(d["deadline_date"])
        days_left = (deadline_date - today).days
        if days_left < 0:
            col.metric(d["name"], "Past due", delta=f"{-days_left} days ago", delta_color="inverse")
        else:
            col.metric(d["name"], f"{days_left} days left", delta=deadline_date.strftime("%b %d, %Y"))

with st.expander("Manage deadlines"):
    with st.form("add_deadline_form", clear_on_submit=True):
        dcol1, dcol2 = st.columns(2)
        deadline_name = dcol1.text_input("Deadline name", placeholder="e.g. Literature review")
        deadline_date_input = dcol2.date_input("Date", value=date.today())
        if st.form_submit_button("Add deadline"):
            if deadline_name.strip():
                add_deadline(deadline_name.strip(), deadline_date_input.isoformat())
                st.rerun()
            else:
                st.error("Please enter a name for the deadline.")

    for d in deadlines:
        c1, c2 = st.columns([4, 1])
        c1.write(f"{d['name']} - {d['deadline_date']}")
        if c2.button("Remove", key=f"del_deadline_{d['id']}"):
...
page = page.split(" ", 1)[1]  # strip the icon for the routing checks below
        ...
        for i, event in enumerate(events):
            with st.container(border=True):
                ...
                skill_tag = st.text_input(
                    "Skill tag (optional)", key=f"skill_{i}", placeholder="e.g. SQL joins, XGBoost"
                )

                b1, b2, b3 = st.columns(3)
                if b1.button("Done", key=f"done_{i}", use_container_width=True):
                    add_entry(event["goal"], event["title"], "done", hours, note, skill_tag=skill_tag)
                    st.success("Logged as done")
                if b2.button("In progress", key=f"inprog_{i}", use_container_width=True):
                    add_entry(event["goal"], event["title"], "in_progress", hours, note, skill_tag=skill_tag)
                    st.success("Logged as in progress")
                if b3.button("Skipped", key=f"skip_{i}", use_container_width=True):
                    add_entry(event["goal"], event["title"], "skipped", hours, note, skill_tag=skill_tag)
...
# ----------------------------------------------------------------
    ...
    with st.form("log_form", clear_on_submit=True):
        ...
        skill_tag = st.text_input(
            "Skill tag (optional)",
            placeholder="e.g. SQL joins, XGBoost, time series",
            help="Builds a running skills inventory over time - view it on the Skills page.",
        )
        ...
        if submitted:
            if not task.strip():
                ...
                add_entry(goal, task, status, hours_spent, notes, log_date.isoformat(), course, skill_tag)
    ...
    if entries:
        ...
        st.dataframe(
            df[["log_date", "goal", "course", "skill_tag", "task", "status", "hours_spent", "notes"]].head(15),
...
# ----------------------------------------------------------------
# PAGE: SKILLS (skill tag inventory)
# ----------------------------------------------------------------
elif page == "Skills":
    st.subheader("Skills inventory")
    st.caption(
        "A running log of what you've actually practiced, in hours - useful for CV bullets "
        "and interview answers ('logged 40 hours on gradient boosting, here's the project')."
    )

    entries = get_all_entries()
    tagged = [e for e in entries if e.get("skill_tag") and e["skill_tag"].strip()]

    if not tagged:
        st.info(
            "No skill tags logged yet. Add one next time you log a task - "
            "e.g. 'SQL joins' or 'XGBoost' - and it will show up here."
        )
    else:
        ...
        # --- Hours per skill tag ---
        st.markdown("**Hours per skill**")
        hours_by_skill = (
            df.groupby("skill_tag")["hours_spent"].sum().sort_values(ascending=False)
        )
        st.bar_chart(hours_by_skill)

        st.divider()
        st.markdown("**Skill details**")
        for skill, group in df.groupby("skill_tag"):
            total_hours = round(group["hours_spent"].sum(), 1)
            first_logged = group["log_date"].min().strftime("%b %d, %Y")
            last_logged = group["log_date"].max().strftime("%b %d, %Y")
            with st.expander(f"{skill} - {total_hours}h across {len(group)} sessions"):
                st.caption(f"First logged {first_logged} · most recent {last_logged}")
                for _, row in group.sort_values("log_date", ascending=False).iterrows():
                    st.markdown(f"**{row['log_date'].strftime('%b %d, %Y')}** - {row['task']} ({row['goal']})")
                    if row["notes"]:
        ...
        for app_row in applications:
            with st.container(border=True):
                ...
                with c1:
                    ...
                    st.markdown(status_badge(app_row["status"]), unsafe_allow_html=True)
                    if app_row["notes"]:
                ...
                for goal, group in notes_df.groupby("goal"):
                    ...
                    with st.expander(f"{goal} - {group_hours}h logged ({len(group)} entries)"):
                        for _, row in group.sort_values("log_date", ascending=False).iterrows():
                            ...
                            st.caption(f"{row['hours_spent']}h · {row['notes']}")

            # --- Weekly reflection prompt (only shown for the "This week" view) ---
            if period == "This week":
                st.divider()
                st.markdown("**Weekly reflection**")
                st.caption("Data shows what happened. This is where it turns into a decision.")

                week_key = start.isoformat()  # Monday's date uniquely identifies the week
                existing_reflection = get_reflection(week_key) or ""

                reflection_text = st.text_area(
                    "What's one thing to change next week?",
                    value=existing_reflection,
                    key=f"reflection_{week_key}",
                    placeholder="e.g. Move DataCamp earlier in the day, I keep skipping it by 2pm",
                )
                if st.button("Save reflection"):
                    if reflection_text.strip():
                        save_reflection(week_key, reflection_text.strip())
                        st.success("Reflection saved")
                    else:
                        st.error("Write something before saving.")

                past_reflections = [r for r in get_all_reflections() if r["week_start"] != week_key]
                if past_reflections:
                    with st.expander("Past reflections"):
                        for r in past_reflections:
                            week_label = date.fromisoformat(r["week_start"]).strftime("Week of %b %d, %Y")
