...
def init_db():
    ...
    existing_app_columns = [row[1] for row in cursor.fetchall()]
    if "required_skills" not in existing_app_columns:
        cursor.execute("ALTER TABLE applications ADD COLUMN required_skills TEXT")
    cursor.execute(
        """
    ...
    # "Personal life" was added after the initial seed, so add it separately
    # (safe no-op if it's already there, e.g. on a fresh database).
    cursor.execute(
        "INSERT OR IGNORE INTO goals (name, target_hours) VALUES (?, NULL)", ("Personal life",)
    )
    conn.commit()
    ...
    # Seed Project 1's deliverable checklist, once, if it has no subtopics yet.
    # These are portfolio artifacts, not hours - what actually gets shown to
    # employers, framed around the C-MAPSS predictive maintenance project.
    cursor.execute(
        "SELECT COUNT(*) FROM subtopics WHERE goal_name = 'Project 1: Battery Life'"
    )
    if cursor.fetchone()[0] == 0:
        deliverables = [
            "Clean & explore the C-MAPSS dataset",
            "Engineer sensor/degradation features",
            "Build a baseline RUL prediction model",
            "Tune and validate the model (proper evaluation metric)",
            "Write up results with e-mobility/off-grid framing",
            "Deploy a simple demo (API or Streamlit app)",
            "Polish the GitHub repo + README",
            "Publish a LinkedIn writeup of the project",
        ]
...
def add_application(company, role, status="applied", job_link="", notes="", date_applied=None, required_skills=""):
    ...
    cursor.execute(
        """
        INSERT INTO applications (date_applied, company, role, status, job_link, required_skills, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (date_applied, company, role, status, job_link, required_skills, notes),
    )
    conn.commit()
    conn.close()
