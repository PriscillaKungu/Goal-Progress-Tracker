...
Note: your calendar blocks are recurring (weekly on weekdays), so this
uses recurring_ical_events to properly expand recurrence rules (RRULE)
for the target date - a plain icalendar walk only sees the first
occurrence's date and misses every later one.
"""
...
def fetch_today_events(ical_url, timezone_name="Africa/Nairobi"):
    ...
    Properly expands recurring events (weekly, daily, etc.) for today's date.
    """
    ...
    # .between() expands all RRULE/EXDATE/RDATE recurrence for the given
    # window and returns concrete occurrences - this is what actually finds
    # "today's" instance of a recurring weekly event.
